...
async def dogwatch(request):
    content = open(os.path.join(ROOT, 'client/dogwatch-cam.svg'), 'r').read()
    return web.Response(content_type='image/svg+xml', text=content)

async def dogwatch_logo(request):
    content = open(os.path.join(ROOT, 'client/dogwatch-logo.svg'), 'r').read()
    return web.Response(content_type='image/svg+xml', text=content)
...
async def playAudio(request):
    ...
    for key in iter(req):
        print(key) 
    print('\nPlay Audio Triggered')
    # Open file and write binary (blob) data
    f = open('./file.wav', 'wb')
    f.write(req['data'].file.read())

    print('\nPlay Audio Triggered 2')
...
if __name__ == '__main__':
    ...
    app.router.add_get('/dogwatch-logo.svg', dogwatch_logo)
    app.router.add_get('/dogwatch-cam.svg', dogwatch)
